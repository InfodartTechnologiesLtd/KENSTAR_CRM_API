...
public class AuthController {
	...
	public ResponseEntity<ResponseDto<PinDto>> forgotPin(@PathVariable Long userId, @RequestBody PinDto pinDto) {
		PinDto updatedPinDto = authService.forgotPin(userId, pinDto);
		ResponseDto<PinDto> response = ResponseDto.success("200",
				"Default PIN 1234 updated successfully. Please change it after login.", updatedPinDto);
		return ResponseEntity.ok(response);
	}

	@PutMapping("/changePin/{userId}")
	@Operation(summary = "Change PIN", description = "Change user PIN after verifying old PIN")
	public ResponseEntity<ResponseDto<PinDto>> changePin(@PathVariable Long userId, @RequestBody PinDto requestDto) {
		PinDto updatedPin = authService.changePin(userId, requestDto);
		ResponseDto<PinDto> response = ResponseDto.success("200", "PIN changed successfully", updatedPin);
		return ResponseEntity.ok(response);
	}
